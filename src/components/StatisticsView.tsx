  ...
  return (
    <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 bg-slate-50/50 dark:bg-slate-900/50 pb-16">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-blue-50 dark:bg-blue-950/60 border border-blue-200/50 dark:border-blue-800/50 text-blue-600 dark:text-blue-400 text-xs font-bold">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-white dark:bg-slate-850 rounded-2xl p-4 shadow-xs border border-slate-200/70 dark:border-slate-800 text-center">
      ...
      <div className="bg-white dark:bg-slate-850 rounded-2xl p-4 shadow-xs border border-slate-200/70 dark:border-slate-800">
        ...
        {next7DaysTasks.length === 0 ? (
          <p className="text-xs text-slate-400 italic py-2">No urgent tasks in the next 7 days</p>
        ) : (
          <div className="space-y-2.5">
            {next7DaysTasks.map((t) => (
              <button
                ...
                className="w-full flex items-center justify-between text-left hover:bg-slate-50 dark:hover:bg-slate-800/60 p-1.5 rounded-xl transition-colors group"
              >
                <div className="flex items-center gap-2 truncate">
                  <Calendar className="w-4 h-4 text-slate-400 shrink-0 group-hover:text-blue-500" />
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-semibold py-1 pl-2.5 pr-6 rounded-lg outline-hidden"
            >
        ...
        <div className="flex items-center justify-around gap-4">
          ...
          <div className="relative w-28 h-28 shrink-0 flex items-center justify-center">
            <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
              ...
              <circle cx="50" cy="50" r="24" className="fill-white dark:fill-slate-850" />
            </svg>
      ...
      <div className="bg-white dark:bg-slate-850 rounded-2xl p-4 shadow-xs border border-slate-200/70 dark:border-slate-800">
        ...
        <div className="relative pt-6 pb-2">
          {/* Y Axis Guide Lines */}
          <div className="absolute inset-0 flex flex-col justify-between pointer-events-none text-[9px] text-slate-400 pl-1">
            <div className="border-b border-slate-100 dark:border-slate-800 w-full flex items-center">
              <span>20</span>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 w-full flex items-center">
              <span>10</span>
          ...
          <div className="grid grid-cols-7 gap-2 h-32 items-end pl-6 pr-2">
            {dailyCompletionData.map((bar, i) => (
              <div key={i} className="flex flex-col items-center gap-1.5 h-full justify-end group">
                <div
                  className="w-full max-w-[20px] bg-blue-500/80 group-hover:bg-blue-600 rounded-t-md transition-all relative"
                  style={{ height: `${bar.percent}%` }}
                >
                  <span className="opacity-0 group-hover:opacity-100 absolute -top-5 left-1/2 -translate-x-1/2 text-[9px] font-bold bg-slate-800 text-white px-1 rounded transition-opacity">
                    {bar.count}
                  </span>
                </div>
