...
export const BottomNavBar: React.FC<BottomNavBarProps> = ({
  ...
  return (
    <nav aria-label="Bottom Navigation" className="w-full bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-200/80 dark:border-slate-800/80 py-1 px-2 flex items-center justify-around z-20 shadow-xs">
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            <div className="relative">
              {tab.icon}
              {tab.badge !== undefined && (
                <span className="absolute -top-1.5 -right-2.5 bg-blue-500 text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">
